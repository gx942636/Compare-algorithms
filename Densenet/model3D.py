...
class DenseNet(nn.Module):
    ...
    def _create_network(self):
        c_hidden = self.hparams.in_filters  # 初始通道数
        self.conv = clones(
            nn.Conv2d(in_channels=1, out_channels=c_hidden, kernel_size=3, padding=1, bias=False),
            5
        )
        self.denseblock = clones(
            DenseBlock(c_in=c_hidden,
                       num_layers=4,  # 可以是 self.hparams.num_layers[block_idx]，但要保证传入正确
        ...
        self.transitionlayer = clones(
            TransitionLayer(c_in=c_hidden + 4 * self.hparams.growth_rate,  # 4 是假设的 num_layers，* growth_rate
                           c_out=1,
                           act_fn=self.hparams.act_fn),
            5
        )
