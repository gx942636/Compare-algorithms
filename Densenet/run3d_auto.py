...
def net_test_2d(net=None, device=None, current_xulie=None):
    ...
    # 加载对应的理想数据用于显示和保存
    path = os.path.join("A3DK08_r_slices", f"label_spec{current_xulie}.mat")
    ...
    file_path_recon = f'./A3DK08_r_slices_recon/{recon_name}.mat'
    savemat(file_path_recon, {'recon_spec': recon_spec})
    label_name = f'label_spec{xulie}'
    file_path_label = f'./A3DK08_r_slices_recon/{label_name}.mat'
...
if __name__ == "__main__":
    ...
    # 加载模型
    print("Loading model...")
    net = torch.load(r'dense_rate0.08_40_64_maskrandom_sumloss.pth')
    net = net.to(device)
    ...
    # 确保输出目录存在
    os.makedirs('./A3DK08_r_slices_recon', exist_ok=True)
